...
try:
    ...
    from salonmax_products.gym_routes import register_gym_routes
except ModuleNotFoundError:
    class salon_product:
        """Temporary fallback if a deploy misses the salon product module."""
...
if "salon_product" not in globals():
    try:
        from salonmax_products import salon as salon_product
    except ModuleNotFoundError:
        class salon_product:
            """Fallback if a deploy misses only the salon product module."""
...
def gym_staff_session_key(business_account_public_id: str) -> str:
    return gym_product.staff_session_key(business_account_public_id)
...
def ensure_gym_staff_auth_table():
    gym_product.ensure_staff_auth_table(platform_execute)


def ensure_gym_payment_settings_table():
    gym_product.ensure_payment_settings_table(platform_execute)


def ensure_gym_checkout_events_table():
    gym_product.ensure_checkout_events_table(platform_execute)


def gym_state_database_url() -> str:
    return gym_product.state_database_url()


def gym_state_pg_connect():
    return gym_product.state_pg_connect()


def ensure_gym_state_table():
    gym_product.ensure_state_table(platform_execute)


def read_gym_business_state(business_account_public_id: str):
    return gym_product.read_business_state(business_account_public_id, platform_execute, platform_query_one)


def write_gym_business_state(business_account_public_id: str, state_data: dict):
    return gym_product.write_business_state(business_account_public_id, state_data, platform_execute, now_utc_text)
...
def ensure_default_gym_business():
    return gym_product.ensure_default_business(
        ensure_platform_sync_tables=ensure_platform_sync_tables,
        platform_execute=platform_execute,
        platform_query_one=platform_query_one,
        generate_password_hash=generate_password_hash,
        now_utc_text=now_utc_text,
    )
...
def sunbed_rows():
    return salon_product.sunbed_rows(
        ensure_sunbed_tables=ensure_sunbed_tables,
        query_all=query_all,
        default_sunbed_label=default_sunbed_label,
        custom_sunbed_name_from_row=custom_sunbed_name_from_row,
        default_catalogue_image_path=default_catalogue_image_path,
    )


def business_settings_row():
    return salon_product.business_settings_row(
        ensure_business_settings_table=ensure_business_settings_table,
        query_one=query_one,
    )
...
def best_seller_rows(start_text, end_text):
    return salon_product.best_seller_rows(query_all=query_all, start_text=start_text, end_text=end_text)


def parse_date_or_default(value: str, fallback_date):
    return salon_product.parse_date_or_default(value, fallback_date)


def transactions_for_day(day_value):
    return salon_product.transactions_for_day(query_all=query_all, day_value=day_value)
...
def gym_payment_settings(business_account_public_id: str):
    return gym_product.payment_settings(business_account_public_id, platform_execute, platform_query_one)


def verify_stripe_signature(payload: bytes, signature_header: str, webhook_secret: str, tolerance_seconds: int = 300) -> bool:
    return gym_product.verify_stripe_signature(payload, signature_header, webhook_secret, tolerance_seconds)


def save_gym_checkout_event_from_session(event_id: str, checkout_session: dict, raw_event: dict | None = None):
    return gym_product.save_checkout_event_from_session(
        event_id,
        checkout_session,
        platform_execute=platform_execute,
        platform_query_one=platform_query_one,
...
def create_stripe_checkout_session(stripe_secret_key: str, connected_account_id: str, params: dict):
    return gym_product.create_stripe_checkout_session(stripe_secret_key, connected_account_id, params)


def retrieve_stripe_checkout_session(stripe_secret_key: str, connected_account_id: str, checkout_session_id: str):
    return gym_product.retrieve_stripe_checkout_session(stripe_secret_key, connected_account_id, checkout_session_id)
...
register_gym_routes(
    app,
    {
        ...
        "default_gym_business_public_id": default_gym_business_public_id,
        "ensure_default_gym_business": ensure_default_gym_business,
        "ensure_gym_checkout_events_table": ensure_gym_checkout_events_table,
        "ensure_gym_staff_auth_table": ensure_gym_staff_auth_table,
        "gym_payment_settings": gym_payment_settings,
        "gym_staff_login_redirect": gym_staff_login_redirect,
        "gym_staff_session_key": gym_staff_session_key,
        "gym_state_database_url": gym_state_database_url,
        "json_error": json_error,
        "now_utc_text": now_utc_text,
        "platform_execute": platform_execute,
        "platform_query_one": platform_query_one,
        "read_gym_business_state": read_gym_business_state,
        "retrieve_stripe_checkout_session": retrieve_stripe_checkout_session,
        "salonmax_public_gym_snapshot": salonmax_public_gym_snapshot,
        "save_gym_checkout_event_from_session": save_gym_checkout_event_from_session,
        "verify_stripe_signature": verify_stripe_signature,
        "write_gym_business_state": write_gym_business_state,
    },
)

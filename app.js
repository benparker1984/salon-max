  ...
  tenants: [
    {
      ...
      billingDay: 1,
      adminNotes: "KADO Fitness launch candidate.",
...
const STATE_API_URL = `/gym/${encodeURIComponent(BUSINESS_ID)}/state`;
...
function renderTenantStatus() {
  ...
  els.systemHint.textContent = suspended ? "Access blocked" : "Check-in ready";
...
async function completeCheckout(event) {
  ...
  try {
    ...
    if (result.payment_mode === "setup_required" && result.demo_allowed) {
      ...
      renderPackagePurchaseMessage(`${plan.name} added for this guided test. Live Stripe checkout is not switched on yet.`);
    } else if (!response.ok) {
...
async function saveRemoteState() {
  try {
    ...
    console.warn("Gym state could not be saved to the server yet.", error);
  }
}
...
async function hydrateRemoteState() {
  if (!BUSINESS_ID) return;
  try {
    const response = await fetch(STATE_API_URL, { headers: { Accept: "application/json" } });
    ...
    console.warn("Gym state could not be loaded from the server yet.", error);
  }
}
